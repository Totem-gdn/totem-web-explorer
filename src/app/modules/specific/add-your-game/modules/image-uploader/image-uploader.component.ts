...
export class ImageUploaderComponent implements OnInit, OnDestroy {
  filesToUpload!: { images: ImagesToUpload, gameSubmitResponse: SubmitGameResponse, jsonFile?: JsonDNAFilters | null };
  ...
  constructor(
    ...
    @Inject(MAT_DIALOG_DATA) public data: { images: ImagesToUpload, gameSubmitResponse: SubmitGameResponse, jsonFile?: JsonDNAFilters | null },
  ...
  ngOnInit() {
    ...
    (this.data.jsonFile?.gameDNA ? 1 : 0) +
    (this.data.jsonFile?.itemDNA ? 1 : 0) +
    (this.data.jsonFile?.avatarDNA ? 1 : 0);

    this.linkImagesToGame(this.filesToUpload);
    //this.submitGameService.approveGame(this.filesToUpload.gameSubmitResponse.id);
  }

  linkImagesToGame(data: { images: ImagesToUpload, gameSubmitResponse: SubmitGameResponse, jsonFiles?: JsonDNAFilters | null }) {
